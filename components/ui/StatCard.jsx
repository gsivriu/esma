...
export default function StatCard({ value, label, detail, countRunId = 0, glass = false }) {
  ...
  return (
    <Card className={`h-full${glass ? " card-glass" : ""}`}>
      <p className={`stat-number font-numbers text-3xl md:text-5xl font-medium leading-tight ${glass ? "text-[var(--color-text-inverse)]" : "text-[var(--color-text)]"}`}>{displayValue}</p>
      <p className={`stat-label font-body text-xs font-medium tracking-[0.12em] uppercase mt-4 font-medium ${glass ? "text-[var(--color-text-inverse)]" : "text-[var(--color-text-muted)]"}`}>{label}</p>
      {detail ? <p className={`stat-description type-body mt-3 ${glass ? "text-[var(--color-text-inverse)]" : "text-[var(--color-text-muted)]"}`}>{detail}</p> : null}
    </Card>
  );
}
